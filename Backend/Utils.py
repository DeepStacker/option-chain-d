...
class Utils:
        ...
        try:
            ...
            for key, value in oc_dict.items():
                try:
                    ...
                    print(f"Skipping invalid key '{key}': Cannot be converted to float.")
    ...
    def find_strikes(option_chain, atm_price):
        try:
            ...
            nearest_strike = min(valid_strikes, key=lambda strike: abs(strike - atm_price))
    ...
    def fetch_percentage(option_chain):
        ...
        def calculate_percentage(value, max_value):
            return (round((value / max_value * 100),2) if max_value else 0) if value > 0 else 0

        # Calculate percentages
        ce_oi_percentage = [calculate_percentage(v, max(ce_oi)) for v in ce_oi]
        ce_oichng_percentage = [calculate_percentage(v, max(ce_oichng)) for v in ce_oichng]
        ...
        pe_oichng_percentage = [calculate_percentage(v, max(pe_oichng)) for v in pe_oichng]
        pe_vol_percentage = [calculate_percentage(v, max(pe_vol)) for v in pe_vol]

        # Calculate percentages
        ce_oi_max_value = [check_data(item=v, value= ce_oi_percentage) for v in ce_oi_percentage]
        ce_oichng_max_value = [check_data(item=v, value= ce_oichng_percentage) for v in ce_oichng_percentage]
        ce_vol_max_value = [check_data(item=v, value= ce_vol_percentage) for v in ce_vol_percentage]
        pe_oi_max_value = [check_data(item=v, value= pe_oi_percentage) for v in pe_oi_percentage]
        pe_oichng_max_value = [check_data(item=v, value= pe_oichng_percentage) for v in pe_oichng_percentage]
        pe_vol_max_value = [check_data(item=v, value= pe_vol_percentage) for v in pe_vol_percentage]
