        ...
        if str(exp) in data.keys():
            for key, value in dict(data[str(exp)][str(curr_date)]).items():
                ...
                if strike in percent_data:
                    ...
                    oi.append(
                        percent_data[strike].get("OI_percentage", 0)
                    )  # Default to 0 if not found
                    oichng.append(
                        percent_data[strike].get("oichng_percentage", 0)
                    )  # Default to 0 if not found
                    vol.append(
                        percent_data[strike].get("vol_percentage", 0)
                    )  # Default to 0 if not found
                else:
...
get_percentage_data(
    symbol="NIFTY", exp=1414175400, curr_date=1729535400, isCe=True, strike="24600"
)
