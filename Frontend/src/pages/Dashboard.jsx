...
const Dashboard = () => {
  ...
  const user = getAuthToken() ? JSON.parse(getAuthToken()) : null; // Parse only if the user exists
  const username = user?.username;

  // console.log(username); // Logs the username or `undefined` if not found

  const cards = [
    {
      title: "Option Chain",
      description:
        "View and analyze option chain data with advanced filters and real-time updates.",
    ...
    },
    {
      title: "Risk Analysis",
      description:
        "Calculate potential profit and loss scenarios for your trades.",
    ...
    },
    {
      title: "Blog",
      description:
        "Read latest articles about trading strategies and market insights.",
      icon: <FaBlog size={24} />,
      link: "/blog",
      color: "purple",
    },
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          {cards.map((card) => (
            ...
            >
              <div
                className={`inline-block p-3 rounded-lg bg-${card.color}-100 text-${card.color}-600 mb-4`}
              >
                {card.icon}
              </div>
